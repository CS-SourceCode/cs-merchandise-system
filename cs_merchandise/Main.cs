...
namespace cs_merchandise
{
    public partial class Main : Form
    {
        ...
        {
            dgvMerch.DataSource = conn.Select("merchandise", "*").GetQueryData();
        }

        private void btnAddMerch_Click_1(object sender, EventArgs e)
        {
            conn.Insert("merchandise", "merch_name", txtMerchName.Text, "merch_price", txtMerchPrice.Text).GetQueryData();
            txtMerchName.Text = "";
            txtMerchPrice.Text = "";
            refreshMerchandise();
        }

        private void dgvMerch_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
           if (e.RowIndex > -1)
           {
              txtMerchID.Text = dgvMerch.Rows[e.RowIndex].Cells["merch_id"].Value.ToString();
              txtMerchName.Text = dgvMerch.Rows[e.RowIndex].Cells["merch_name"].Value.ToString();
              txtMerchPrice.Text = dgvMerch.Rows[e.RowIndex].Cells["merch_price"].Value.ToString();
              btnAddMerch.Visible = false;
              btnUpdateMerch.Visible = true;
            }
            
        }

        private void btnClearMerch_Click(object sender, EventArgs e)
        {
            txtMerchID.Text = "";
            txtMerchName.Text = "";
            txtMerchPrice.Text = "";
            btnAddMerch.Visible = true;
            btnUpdateMerch.Visible = false;
        }

        private void btnUpdateMerch_Click(object sender, EventArgs e)
        {
            conn.Update("merchandise", "merch_name", txtMerchName.Text, "merch_price", txtMerchPrice.Text).Where("merch_id", txtMerchID.Text).GetQueryData();
            refreshMerchandise();
        }

        private void btnSearchMerch_Click(object sender, EventArgs e)
        {
            dgvMerch.DataSource = conn.Select("merchandise", "*").Like("merch_name", txtSearchMerch.Text).GetQueryData();
        }
    }
}
